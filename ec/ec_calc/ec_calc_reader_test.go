...
func TestECCalc_ECReader(t *testing.T) {
	...
	for i := 0; i < 32; i++ {
		...
		fid[0], err = se.PutObject(ctx, sz[0], bytes.NewReader(b[:sz[0]]))
		assert.Equal(t, err, nil)

		fid[1], err = se.PutObject(ctx, sz[1], bytes.NewReader(b[sz[0]:sz[0]+sz[1]]))
		assert.Equal(t, err, nil)

		fid[2], err = se.PutObject(ctx, sz[2], bytes.NewReader(b[sz[0]+sz[1]:]))
		assert.Equal(t, err, nil)
